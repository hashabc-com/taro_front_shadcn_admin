...
/**
 * Hook to simplify Google Auth dialog usage.
 *
 * Usage:
 * ```tsx
 ...
 *     await someApi({ gauthKey, ...otherParams })
 *   })
 * }
 *
 * return <>{dialog}</>
...
export function useGoogleAuthDialog() {
  ...
  const [callback, setCallback] = useState<
    ((gauthKey: string) => void | Promise<void>) | null
  >(null)

  const withGoogleAuth = useCallback(
    (cb: (gauthKey: string) => void | Promise<void>) => {
      setCallback(() => cb)
      setOpen(true)
    },
    []
