...
export function useCountryDailySummaryData() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['fund', 'country-daily-summary', search, selectedCountry?.code, selectedMerchant?.appid],
    queryFn: () => getCountryDailySummary(search),
    enabled: !!selectedCountry,
    placeholderData:(prev) => prev ?? undefined
  })

  const dataList = data?.result?.listRecord?.map((item: ICountryDailySummaryType) => ({
    ...item,
