...
export function AccountMutateDrawer({
  ...
  const accountFormSchema = z.object({
    userName: z
      .string()
      .min(1, ti18n('system.accountManage.validation.nameRequired')),
    account: z
      .string()
      .min(1, ti18n('system.accountManage.validation.accountRequired')),
    password: z.string().optional(),
    mobile: z.string().optional(),
    roleIds: z.number().or(z.string()),
    userType: z
      .number()
      .min(1, ti18n('system.accountManage.validation.typeRequired')),
  ...
  const form = useForm<AccountFormValues>({
    resolver: zodResolver(
      isAdd
        ? accountFormSchema.extend({
            password: z
              .string()
              .min(
                1,
                ti18n('system.accountManage.validation.passwordRequired')
              ),
          })
        : accountFormSchema
    ),
  ...
  const roleList =
    rolesData?.result?.map((item: { role: string; id: number }) => ({
      label: item.role,
      value: item.id,
    })) || []
  ...
  return (
    <Sheet open={open} onOpenChange={handleClose}>
      <SheetContent className='flex flex-col overflow-y-auto sm:max-w-[540px]'>
        <SheetHeader className='text-start'>
          ...
          <SheetDescription>
            {isAdd
              ? ti18n('system.accountManage.createDescription')
              : ti18n('system.accountManage.editDescription')}
          </SheetDescription>
        </SheetHeader>
        ...
        <Form {...form}>
          <form
            ...
            className='flex-1 space-y-10 px-4'
          >
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder={ti18n(
                        'system.accountManage.placeholder.name'
                      )}
                      {...field}
                    />
                  </FormControl>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder={ti18n(
                        'system.accountManage.placeholder.account'
                      )}
                      {...field}
                    />
                  </FormControl>
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        type='password'
                        placeholder={ti18n(
                          'system.accountManage.placeholder.password'
                        )}
                        {...field}
                      />
                    </FormControl>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder={ti18n(
                        'system.accountManage.placeholder.phone'
                      )}
                      {...field}
                    />
                  </FormControl>
                ...
                <FormItem className='flex items-center justify-between rounded-lg border p-4'>
                  <div className='space-y-0.5'>
                    <FormLabel className='text-base'>
                      {ti18n('system.accountManage.accountStatus')}
                    </FormLabel>
                    <div className='text-muted-foreground text-sm'>
                      {field.value === 0
                        ? ti18n('common.enabled')
                        : ti18n('common.disabled')}
                    </div>
                  </div>
                  <FormControl>
                    <Switch
